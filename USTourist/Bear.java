...
public class Bear implements Runnable, Serializable {

    private transient MyHashTable<Location, GridObject> table;
    private int row, col, squareSize;
    // private BufferedImage img;

    public Bear(MyHashTable<Location, GridObject> grid, int r, int c, int ss) {
        table = grid;
        row = r;
        col = c;
        squareSize = ss;
        ...
        while (true) {
            ...
            if (direction == 1) {
                if (!table.get(new Location(row - 1, col)).get(1)
                        ...
                        && !table.get(new Location(row - 1, col)).get(1)
                                .equals(new GridObject("water", Color.black, squareSize))
                        && table.get(new Location(row - 1, col)).size() < 3) {
                    row--;
                    // y-=squareSize;
                } else {
                    continue;
            ...
            if (direction == 3) {
                if (!table.get(new Location(row, col + 1)).get(1)
                        .equals(new GridObject("road", Color.black, squareSize))
                        && !table.get(new Location(row, col + 1)).get(1)
                                .equals(new GridObject("water", Color.black, squareSize))
                        && table.get(new Location(row, col + 1)).size() < 3) {
                    col++;
                    // x+=50;
                } else {
                    continue;
